...
class PurchaseV3(models.Model):
    ...
    user = models.OneToOneField(User, on_delete=models.CASCADE, null=True, related_name='purchase')
    ...
    @property
    def pending_payment_is_done(self) -> bool:
        status = ops.payment_status(self.transaction_id)
        try: _ = self.close_transaction() if status else None
        except: pass
        return status
    ...
    def close_transaction(self):
        if not self.is_closed:
            self.is_closed = True
            self.save()
            self.user.status.upgrade_to_premium()
