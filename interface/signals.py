...
@receiver(pre_save, sender=payment_models.PaymentTracker)
def ensure_unique_tracker_key(
    instance: payment_models.PaymentTracker, *args, **kwargs):
    tracker = instance
    if not tracker.key:
        generator = keygen.KeyGen()
        def generate_key() -> str:
            return (generator.alphanumeric_key(10) 
                + generator.datetime_key() 
                    + generator.alphanumeric_key(10))
        key = generate_key()
        while payment_models.PaymentTracker.objects.filter(
            key=key).exists():
            key = generate_key() 
        tracker.key = key
